function is_path(input_string) {
    if (typeof input_string !== 'string') return 'unknown';
    ...
    const is_windows = /^[a-zA-Z]:[\\\/]/.test(input_string);
    const is_unix = input_string.startsWith('/');
    const is_relative = input_string.startsWith('./') || input_string.startsWith('../');
    const has_path = input_string.includes('/') || input_string.includes('\\');

    if (is_windows || is_unix || is_relative || has_path) {
        return 'local_path';
    }
...
// Source - https://stackoverflow.com/a/7544757
...
function replace_all_rel_by_abs(html, base_url){
    ...
    */
    function by(match, group1, group2, group3){
        ...
        * return group1 + "javascript://" + group3; */
        return group1 + rel_to_abs(group2, base_url) + group3;
    }
    /*
    @name by2
